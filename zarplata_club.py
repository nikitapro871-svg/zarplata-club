...
# === HTML КОД (ТОТ ЖЕ, ЧТО И РАНЬШЕ, НО Я СОКРАТИЛ ДЛЯ ЭКОНОМИИ МЕСТА) ===
# Полный HTML-код я вставлю отдельно, чтобы не перегружать сообщение.
# Напишите "дай HTML", и я пришлю его отдельно.
...
def index():
    ...
    if session.get('user_id'):
        user_row = conn.execute("SELECT is_admin FROM employees WHERE id=?", (session['user_id'],)).fetchone()
        if user_row and user_row[0] == 1:
            user_is_admin = True
    
    if user_is_admin or session.get('is_admin'):
        # Админ видит всех
...
def employee_detail(emp_id):
    ...
    if not is_admin and session['user_id'] != emp_id:
        conn.close()
...
def login():
    ...
    session['is_admin'] = bool(employee[3])  # Если в базе is_admin = 1, то админ
...
def make_admin():
    # Только главный админ (admin) может давать права админа
...
def add_hours():
    ...
    # Проверка прав: только админ может добавлять часы
    if not session.get('is_admin'):
        conn = get_db()
        user_row = conn.execute("SELECT is_admin FROM employees WHERE id=?", (session['user_id'],)).fetchone()
        conn.close()
